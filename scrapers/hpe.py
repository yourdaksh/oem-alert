"""
HP/HPE Security Bulletins scraper
"""
import re
...
class HPEScraper(RSSScraper):
    """Scraper for HP/HPE Security Bulletins"""
    
    def scrape_vulnerabilities(self) -> List[Dict[str, Any]]:
        """Scrape HP/HPE vulnerabilities"""
        vulnerabilities = []
        ...
        if vuln_url:
            ...
            if soup:
                vulnerabilities.extend(self.parse_hpe_page(soup))
        
        return vulnerabilities
    
    def parse_hpe_page(self, soup) -> List[Dict[str, Any]]:
        """Parse HP/HPE security bulletins page"""
        vulnerabilities = []
        ...
        try:
            ...
            for cve_text in cve_texts:
                try:
                    ...
                    logger.error(f"Error parsing HP/HPE CVE: {e}")
                    continue
        
        except Exception as e:
            logger.error(f"Error parsing HP/HPE page: {e}")
