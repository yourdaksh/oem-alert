      ...
      <aside className="glass" style={{ width: '260px', borderRight: '1px solid var(--surface-border)', display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, bottom: 0, left: 0, zIndex: 50, borderRadius: 0, borderTop: 'none', borderBottom: 'none', borderLeft: 'none' }}>
        ...
        <div style={{ padding: '1.5rem', borderTop: '1px solid var(--surface-border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
            <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'var(--surface-hover)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.9rem', fontWeight: 600 }}>
              {profile?.email?.slice(0, 1).toUpperCase() || '?'}
            </div>
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: '0.85rem', fontWeight: 500, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis' }}>{profile?.email || 'Loading...'}</p>
              <p style={{ fontSize: '0.75rem', color: '#a1a1aa', margin: 0 }}>{profile?.organization?.name || '—'} · {profile?.role}</p>
            </div>
          </div>
          <button onClick={signOut} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', width: '100%', padding: '0.75rem', background: 'transparent', border: 'none', color: '#a1a1aa', cursor: 'pointer', textAlign: 'left', borderRadius: '8px' }}>
            <LogOut size={18} />
        ...
        <header className="glass" style={{ height: '70px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 2rem', position: 'sticky', top: 0, zIndex: 40, borderRadius: 0, borderTop: 'none', borderLeft: 'none', borderRight: 'none' }}>
          <div style={{ position: 'relative', width: '300px' }}>
            ...
            <input type="text" placeholder="Search CVEs, Products..."
              style={{ width: '100%', padding: '0.5rem 1rem 0.5rem 2.5rem', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--surface-border)', borderRadius: '20px', color: 'white', fontSize: '0.9rem', outline: 'none' }} />
          </div>
